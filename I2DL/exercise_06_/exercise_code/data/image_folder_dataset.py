...
class Rotaion90Degree:
    """
    Transform class that rotates images by 90° couter clockwise randomly with a given  probability
    """
...
class SaltPepper:
    """
    Transforms some random pixels to black or white color
    """
    def __init__(self, points=100, activation_threshold=0.5, color_threshold=0.5):
        """
        :param prob: Probability of the image being flipped
        """
        self.points = points
    ...
    def __call__(self, image):
        activation_probability = random.uniform(0,1)
        if activation_probability < self.activation_threshold:
            return image
        else:
            img = np.copy(image) # np array are pased by reference. To keep the original image unchange, we need to modify and pass its copy
            for i in range(1, self.points):
                x = random.randrange(0,31)
                y = random.randrange(0,31)
                back_prob = random.uniform(0,1)
                if back_prob > self.color_threshold:
                    img[x][y][0] = 0
                    img[x][y][1] = 0
                    img[x][y][2] = 0
                else:
