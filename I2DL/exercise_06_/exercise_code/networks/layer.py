...
class Sigmoid:
    ...
    def backward(self, dout, cache):
        ...
        """
        dx = None
        dx = dout * cache * (1 - cache) # Note that derivative of sigmoid is: dσ(x)/dx = 1-σ(x)
        return dx
...
class LeakyRelu:
    ...
    def backward(self, dout, cache):
        ...
        dx[ cache >= 0] = dout[ cache >= 0]
        dx[ cache < 0] = dout[ cache < 0] * self.slope
...
class Tanh:
    ...
    def backward(self, dout, cache):
        ...
        #SM: ∂f/∂x = ∂f/∂g * ∂g/∂x where g is the output and then ∂f/∂g=dout is incomming gradient from left
        dx = dout * (1 - np.power(np.tanh(cache), 2)) 
